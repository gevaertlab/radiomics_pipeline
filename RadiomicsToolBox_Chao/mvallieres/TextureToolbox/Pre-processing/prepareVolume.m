...
%Isotropic voxel size (1). This parameter is denoted as ‘Scale’. Prior 
%to the computation of texture features, all volumes were resampled to 
%an isotropic voxel size set to a desired resolution using cubic 
%interpolation. Scale values of 1 mm, 2 mm, 3 mm, 4 mm, 5 mm and 
%initial in-plane resolution (denoted ‘in-pR’) were tested. For 
%example, if the desired Scale was set to 5 mm, a FDG-PET volume with 
%voxel size of 5.47 × 5.47 × 3.27 mm3 would be isotropically resampled 
%to a voxel size of 5 × 5 × 5 mm3. If the desired Scale was set to 
%in-pR, a MRI volume with voxel size of 0.86 × 0.86 × 5 mm3 would be 
%isotropically resampled to a voxel size of 0.86 × 0.86 × 0.86 mm3. 
%Note that Global texture features are extracted after isotropically 
%resampling to the in-plane resolution without further processing.
...
%    getGlobalTextures(ROIonly,100); % Here, using 100 bins in the histogram.
% -------------------------------------------------------------------------
...
if ~strcmp(textType,'Global') && ~strcmp(textType,'Matrix') && ~strcmp(textType,'No_preprocessing')
    error('Last argument must either be ''Global'' or ''Matrix'' or ''No_preprocessing''')
end
if nargin > 8
...
% WAVELET BAND-PASS FILTERING, R=1 for groups other than wavelet. The code to do wavelet filter here was deprecated and we used the method detailed in Hugo Aerts et al. Nat Com. 2014 to get 8 wavelt transform decompositions in computeWavelet().
% if R ~= 1
%     if sum(isnan(ROIbox(:)))
...
if ~strcmp(textType, 'No_preprocessing')
    flagPW = 0;
...
% QUANTIZATION
%Quantization of gray levels. Prior to the computation of texture 
%features, the full intensity range of the tumour region was quantized 
%to a smaller number of gray levels Ng. The quantization process maps 
%the voxel values of a volume to a finite set r = {r k k ∈ = R : 1, 2, 
%, … } Ng of reconstruction levels by defining a set t = {t k k ∈ = R : 1, 2, , … + Ng 1} of decision levels.

if strcmp(textType,'Matrix')
    [ROIonly,levels] = quantization(ROIonly,Ng);
elseif strcmp(textType,'Global')
    levels = 0;
elseif strcmp(textType,'No_preprocessing')
    levels = NaN;
end
